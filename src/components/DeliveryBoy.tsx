...
function DeliveryBoy() {
  ...
  // ---------------------------------------------------------
  // Fetch delivery boy online/offline status
  // ---------------------------------------------------------
  ...
  // ---------------------------------------------------------
  // Fetch incoming order broadcasts
  // ---------------------------------------------------------
  ...
  useEffect(() => {
    ...
    socket.on('delivery:newBroadcast', (assignment: any) => {
      setBroadcasts((prev) => {
        if (prev.some((b) => b._id === assignment._id)) {
          return prev
        }
    ...
    // -------------------------------------------------------
    // Another delivery boy accepted the order
    // -------------------------------------------------------
    socket.on(
      'delivery:broadcastTaken',
    ...
    // -------------------------------------------------------
    // Our active order was completed
    // -------------------------------------------------------
  ...
  useEffect(() => {
    ...
    if (online) {
      socket.emit('deliveryBoy:online', {
        deliveryBoyId: session.user.id,
      })
  ...
  const toggleOnline = async () => {
    ...
    try {
      await axios.post('/api/delivery/status', {
        isOnline: next,
      })
  ...
  const accept = async (assignmentId: string) => {
    ...
    try {
      await axios.post(
        `/api/delivery/assignment/${assignmentId}/accept`
      )
  ...
  const reject = async (assignmentId: string) => {
    ...
    try {
      await axios.post(
        `/api/delivery/assignment/${assignmentId}/reject`
      )
  ...
  const requestOtp = async () => {
    ...
    try {
      await axios.post(
        `/api/delivery/order/${activeOrder._id}/request-otp`
      )
  ...
  const confirmDelivery = async () => {
    ...
    try {
      await axios.post(
        `/api/delivery/order/${activeOrder._id}/confirm-delivery`,
        {
          code: otpCode,
        }
      )
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {activeOrder ? (
              ...
              <div className="mt-4 space-y-2">
                ...
                {activeOrder.items?.map(
                  (item: any, idx: number) => (
                    <div
                      key={idx}
                      className="flex justify-between text-sm bg-amber-50 rounded-lg px-3 py-2"
                    >
                      <span>
                ...
                {!otpSent ? (
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <p className="text-sm text-gray-600">
                      A confirmation code was emailed to
                      the buyer. Ask them for it and enter
                      below.
                    </p>
          ...
          <div className="bg-white rounded-3xl shadow p-6">
            ...
            {broadcasts.length === 0 ? (
              <p className="text-gray-500 text-sm">
                No new orders right now. Stay online to
                ...
                {broadcasts.map((b) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-600 flex items-center gap-1 mt-1">
                      ...
                      {b.order?.address?.fullAddress}
                    </p>

                    {/* Amount */}
                    <p className="text-xs text-gray-500 mt-1">
                      Rs {b.order?.totalAmount} ·{' '}
                      {b.order?.items?.length || 0} item(s)
                    </p>
                    ...
                    <div className="flex gap-2 mt-3">
                      ...
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1 bg-green-600 hover:bg-green-700 text-white text-sm font-semibold py-2 rounded-lg disabled:opacity-60"
                      >
                        <CheckCircle2 className="w-4 h-4" />
                      ...
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1 bg-white border border-gray-300 text-gray-600 text-sm font-semibold py-2 rounded-lg disabled:opacity-60"
                      >
                        <X className="w-4 h-4" />
          ...
          <div className="bg-white rounded-3xl shadow p-8 text-center text-gray-500">
            You are offline. Toggle "Online" above to start
